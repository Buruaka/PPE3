...
if(!empty($Modifier)){
    ...
    if ($quantitepos==TRUE){
        Updatefichefraisforfait($link,$_SESSION["session_id"],$mois,$listenewquantite,$idvehicule);
    }
    else{
...
if(!empty($Supprimer)){
   Supprimerfichefraisforfait($link,$_SESSION["session_id"],$mois);
...
if(!empty($Modifierhf)){
    ...
    if ($montantpos==TRUE){
        Updatefichehorsforfait($link,$_SESSION["session_id"],$moishf,$libelle,$montant,$idfiche);
    }
    else{
        echo "<h3>erreur: les données entrées ne sont pas positives</h3>";
    }
...
if(!empty($Supprimerhf)){
    Supprimerfichehorsfraisforfait($link,$_SESSION["session_id"],$idfiche);
     ?><h3> Votre fiche de frais a bien été supprimé</h3><?php 
 }
?>

<h3> Vos fiches de frais en forfait : </h3>


<?php
 $fichesfraisforfait = ObtenirFicheFrais($link,$_SESSION['session_id']);

 while( $fichesfraisforfaitliste= mysqli_fetch_assoc($fichesfraisforfait)) { 
     echo $fichesfraisforfaitliste['mois']."<br><br>";
    $fichefrais =Touteficheforfait($link,$_SESSION['session_id'],$fichesfraisforfaitliste['mois']);
...
<form action="VoirFiche.php" method="post">
    ...
    <table class="voirfiche" width="30%">
            ...
            <form action="Genpdf.php" method="post">
                ...
                <input type="hidden" name="visiteur" value=<?php echo $_SESSION['session_id'];?>> 
...
<?php
$fichesfraishorsforfait = ObtenirmoisHorsForfaitFicheFrais($link,$_SESSION['session_id']);
